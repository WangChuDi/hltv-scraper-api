...
def _extract_hltv_date_iso(node):
    ...
    try:
        return datetime.datetime.utcfromtimestamp(int(unix_value) / 1000).date().isoformat()
...
def _build_event_result(href, raw_name):
    ...
    clean_name = re.sub(r'([A-Z][a-z]{2}\s+\d{1,2}(?:st|nd|rd|th)?(?:-[A-Z][a-z]{2}\s+\d{1,2}(?:st|nd|rd|th)?)?).*$', '', clean_name).strip()
...
def get_hltv_event_metadata(event_url):
    try:
        ...
        start_date = _extract_hltv_date_iso(date_nodes[0]) if len(date_nodes) >= 1 else None
        end_date = _extract_hltv_date_iso(date_nodes[-1]) if len(date_nodes) >= 2 else start_date

        location_cell = soup.find('td', class_='location')
        location_text = ' '.join(location_cell.get_text(' ', strip=True).split()) if location_cell else None

        prize_cell = soup.find('td', class_='prizepool')
        prize_text = ''
        if prize_cell:
            prize_title = prize_cell.get('title')
            prize_text = prize_title if isinstance(prize_title, str) else prize_cell.get_text(' ', strip=True)
...
def search_events(query):
    def collect_event_links(soup):
        ...
        for a in soup.find_all('a', href=True):
            href = a.get('href')
            if not isinstance(href, str) or not href.startswith('/events/') or href == '/events/archive':
                continue
            title_node = a.select_one('div.text-ellipsis')
            text = title_node.get_text(' ', strip=True) if title_node else a.get_text(' ', strip=True)
            if text:
    ...
    try:
        ...
        if resp.status_code == 200:
            all_links.extend(collect_event_links(BeautifulSoup(resp.content, 'html.parser')))

        for year in sorted(years):
            archive_url = f'https://www.hltv.org/events/archive?startDate={year}-01-01&endDate={year}-12-31'
            archive_resp = requests.get(archive_url, impersonate='chrome142', timeout=10)
            if archive_resp.status_code == 200:
                all_links.extend(collect_event_links(BeautifulSoup(archive_resp.content, 'html.parser')))
        ...
        for href, text in all_links:
            ...
            if event_result and matches_query(event_result['slug'], event_result['name'], query_parts):
                seen.add(href)
